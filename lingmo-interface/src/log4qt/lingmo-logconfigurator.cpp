...
LingmoLog4qtConfig *LingmoLog4qtConfig::mInstance = nullptr;
pid_t g_MainProcPid = -1;

LingmoLog4qtConfig::LingmoLog4qtConfig(QObject *parent) : 
    QObject(parent),
...
LingmoLog4qtConfig::~LingmoLog4qtConfig()
{
    if (m_gsLog4qtGlobal) {
...
LingmoLog4qtConfig *LingmoLog4qtConfig::instance()
{
    if (!mInstance)
    {
        ...
        if (!mInstance)
        {
            mInstance = new LingmoLog4qtConfig;
        }
    }
    return mInstance;
}

int LingmoLog4qtConfig::init(QString strAppName)
{
    QMutexLocker locker(single_config());
    if (m_isInited) {
        return true;
    ...
    atexit(shutdown);
    // 启动检查日志文件数量和大小线程
    m_threadCheckFile = new LingmoLog4qtRolling(strLogFilePath, m_uMaxFileCount, m_uMaxFileSize, m_timeCheckFile);
...
void LingmoLog4qtConfig::shutdown()
{
    QMutexLocker locker(single_config()); 
    if (mInstance) {
...
void LingmoLog4qtConfig::initSettings(QString strAppName, Log4Qt::Properties &properties)
{
    //读取全局gsettings配置
