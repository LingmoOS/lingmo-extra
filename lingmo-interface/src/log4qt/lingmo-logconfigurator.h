...
// 日志sdk对外衔接类
class LIBLINGMOLOG4QT_EXPORT LingmoLog4qtConfig : public QObject
{
    Q_OBJECT

private:
    virtual ~LingmoLog4qtConfig();
    LingmoLog4qtConfig(QObject *parent = nullptr);

public:
    // 返回日志配置器实例
    static LingmoLog4qtConfig *instance();
    // 初始化日志sdk属性
...
public:
    // 检查日志文件线程对象
    LingmoLog4qtRolling            *m_threadCheckFile;

private:
    // 自身实例
    static LingmoLog4qtConfig     *mInstance;
    // 是否已初始化
    bool                        m_isInited;
    // 全局gsettings
