...
LingmoLog4qtRolling::LingmoLog4qtRolling(QString strBaseFilePath, unsigned uMaxFileCount, quint64 uMaxFileSize, 
...
LingmoLog4qtRolling::~LingmoLog4qtRolling()
{

}

// 线程过程
void LingmoLog4qtRolling::run()
{
    // 时间间隔参数异常
...
// 设置文件检查限制条件
void LingmoLog4qtRolling::setFileCheckLimit(unsigned uMaxFileCount, quint64 uMaxFileSize, quint64 delayCheckFileTime)
{
    m_lockReadWrite.lockForWrite();
    m_uMaxFileCount = uMaxFileCount;
    m_uMaxFileSize = uMaxFileSize*1024*1024; // 单位 M
...
// 停止线程
void LingmoLog4qtRolling::stop()
{
    m_isExit = true;
}

// 检查文件数量
void LingmoLog4qtRolling::checkLogFilesCount()
{
...
// 检查文件大小
void LingmoLog4qtRolling::checkLogFilesSize()
{
    if (m_strBaseFilePath.isEmpty()) {
        return;
    }
